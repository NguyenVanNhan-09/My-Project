...
import { GET_BY_ID_Brand } from '../../../services/Brands'
type Props = {
    id: any
}
...
const UpdateBrand = ({ id }: Props) => {
    ...
    useEffect(() => {
        if (id !== null) {
            ;(async () => {
                try {
                    const data = await GET_BY_ID_Brand(id)
                    reset({
                        name: data.name
                    })
    ...
    const onSubmit = (brand: TBrands) => {
        handleUpdate(id, { ...brand, thumbnail: thumbnail })
    }
    return (
        <>
            <dialog id='modal_update_brand' className='modal'>
